...
const EventDetails = () => {
    ...
    return (
        <>
            {/* Dynamic SEO for Event Pages */}
            {event && (
                <SEOHead
                    title={`${event.title} - Book Tickets`}
                    description={`${event.description?.substring(0, 155) || `Get tickets for ${event.title}`}... Book now on Tickify. ${event.date ? `Date: ${event.date}` : ''} ${event.location ? `Venue: ${event.location}` : ''}`}
                    keywords={[
                        event.title,
                    ...
                    breadcrumbs={[
                        ...
                        { name: event.category || 'Event', url: `/events?cat=${event.category || 'All'}` },
                        { name: event.title }
                    ]}
                />
            )}
